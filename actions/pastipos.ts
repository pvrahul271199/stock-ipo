...
const apiUrl = "https://www.nseindia.com/api/public-past-issues?from_date=07-09-2024&to_date=07-12-2024"; // API URL
const headers = {
    "accept": "*/*",
 ...
 */
export const getCookiesForPastIPOSNSE = async () => {
    try {
        ...
        nseCookies = cookies;
        const nseResponse = await getAPIDataForPastIPOSNSE(nseCookies);
        console.log("NSE Response past IPO:", nseResponse);
        return nseResponse
    } catch (error:any) {
        if (error.status === 403) {
            console.log("getCookies =========> error.status === 403. Retrying...");
            await getCookiesForPastIPOSNSE(); // Retry on 403
        } else {
 ...
 * @returns {Promise<any>} The API response data.
 */
const getAPIDataForPastIPOSNSE = async (nseCookies:any) => {
    try {
        ...
        if (error.status === 401) {
            ...
            if (!nseCookies) {
                ...
                await getCookiesForPastIPOSNSE(); // Fetch cookies
            }
        } else {
