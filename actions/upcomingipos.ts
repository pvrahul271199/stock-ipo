...
const apiUrl = "https://www.nseindia.com/api/all-upcoming-issues?category=ipo"; // API URL
const headers = {
    "accept": "*/*",
 ...
 */
export const getCookiesForUpcomingIPOSNSE = async () => {
    try {
        ...
        nseCookies = cookies;
        const nseResponse = await getAPIDataForUpcomingIPOSNSE(nseCookies);
        console.log("NSE Response upcoming IPO:", nseResponse);
        return nseResponse
    } catch (error:any) {
        if (error.status === 403) {
            console.log("getCookies =========> error.status === 403. Retrying...");
            await getCookiesForUpcomingIPOSNSE(); // Retry on 403
        } else {
 ...
 * @returns {Promise<any>} The API response data.
 */
const getAPIDataForUpcomingIPOSNSE = async (nseCookies:any) => {
    try {
        ...
        const filteredData = data.filter((item: any) => item.series !== "DEBT" && item.status === "Forthcoming");
        return filteredData;
    } catch (error:any) {
        if (error.status === 401) {
            ...
            if (!nseCookies) {
                ...
                await getCookiesForUpcomingIPOSNSE(); // Fetch cookies
            }
        } else {
